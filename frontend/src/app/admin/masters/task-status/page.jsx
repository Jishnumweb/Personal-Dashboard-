...
export default function TaskStatusPage() {
  ...
  const validate = (data) => {
    ...
    if (!data.taskStatus?.trim()) errs.taskStatus = "task status is required";
  ...
  const saveForm = async () => {
    ...
    try {
      if (editingId) {
        // EDIT MODE
        await updateTaskStatus(editingId, form);

        setIsEditOpen(false);
      } else {
        // ADD MODE
        await createTaskStatus(form);
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      await deleteTaskStatus(deletingId);
    ...
    <div className="w-full p-3">
      ...
      {isAddOpen && (
        ...
        >
          <Input
            onChange={(e) =>
              setForm((prev) => ({ ...prev, taskStatus: e.target.value }))
            }
