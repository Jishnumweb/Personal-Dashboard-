...
export default function LeadReferencePage() {
  ...
  const validate = (data) => {
    ...
    if (!data.priority?.trim()) errs.priority = "priority is required";
    return errs;
  };
  ...
  const saveForm = async () => {
    ...
    try {
      if (editingId) {
        // EDIT MODE
        await updatePriority(editingId, form.priority);

        setIsEditOpen(false);
      } else {
        // ADD MODE
        await createPriority(form.priority);
  ...
  const handleDelete = async () => {
    ...
    setSubmitting(true);
    try {
      await deletePriority(deletingId);
          ...
          {priorities.map((p, idx) => (
            <tbody key={idx}>
              <tr className="border-t hover:bg-gray-50 text-center">
                ...
                <td className="px-6 py-4 cursor-pointer">{p.priority}</td>
